...
def main():
    ...
    import pickle
    f = open('/home/zyx/zbb/improved-diffusion/evaluations/tiny_imagetnet_ref_stats.pkl','rb')
    ref_stats = pickle.load(f)
    f.close()
    f = open('/home/zyx/zbb/improved-diffusion/evaluations/tiny_imagetnet_ref_stats_spatial.pkl','rb')
...
class Evaluator_v1(Evaluator):
    def warmup(self):
        # 在这里，通过调用 compute_activations 方法，
        # 传入一个形状为 [1, 8, 64, 64, 3] 的零数组，来进行一次“热身”计算。
        # 这可能有助于预热底层的计算图或模型，确保之后的计算更加准确。
...
def cal_evalutions(batches, batch_size, evaluator,ref_stats_path,ref_stats_spatial_path):
    import pickle
    f = open(ref_stats_path,'rb')
    ref_stats = pickle.load(f)
    f.close()
    f = open(ref_stats_spatial_path,'rb')
    ref_stats_spatial = pickle.load(f)
    f.close()
    print("computing sample batch activations...")
    sample_acts = evaluator.compute_activations(batches, batch_size)
    ...
    iScore = evaluator.compute_inception_score(sample_acts[0])
    return fid, sfid, iScore
